...
class ADPipeline(StableDiffusionXLPipeline):
    ...
    def sample(
        ...
        **kwargs,
        ...
        self.spg_scale=spg_scale
        self.do_spg=True if spg_scale>0.0 else False
        ...
        for i in range(int(50)):
            keeps = [
                1.0 - float(i / int(50) < s or (i + 1) / int(50) > e)
        ...
        if self.do_classifier_free_guidance and not self.do_spg:
            prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds], dim=0)
            add_text_embeds = torch.cat(
                [negative_pooled_prompt_embeds, add_text_embeds], dim=0
            )
            add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)
        if self.do_classifier_free_guidance and self.do_spg:
        ...
        for i, t in enumerate(pbar):
            with torch.no_grad():
                ...
                if self.do_spg:
                    ...
                    self.cache.clear()
                    self.cache.do_spg=False
                    self.cache.do_cfg=False
                    self.cache.do_spg2=False
                ...
                elif self.do_classifier_free_guidance and self.do_spg:
                ...
                if self.do_classifier_free_guidance and self.do_spg:
                    ...
                    noise_pred = noise_pred_uncond+ self.guidance_scale * (
                        noise_pred_text - noise_pred_uncond
                    )+self.spg_scale*(noise_pred_pertube-noise_pred_uncond)
