    ...
    def attn_forward(self):
        def forward(
            ...
            **kwargs,
            ...
            if cache.do_pag and cache.injection and not cache.do_cfg:
                q,q_con=q.chunk(2)
                k,k_con=k.chunk(2)
                v,v_con=v.chunk(2)
            if cache.do_pag and cache.injection and cache.do_cfg and not cache.do_pag2:
                q,q_con,q_uncon=q.chunk(3)
                k,k_con,k_uncon=k.chunk(3)
                v,v_con,v_uncon=v.chunk(3)
            if cache.do_pag and cache.injection and cache.do_cfg and cache.do_pag2:
            ...
            if is_self and cache.injection and controller.cur_self_layer in controller.self_layers:
                ...
                k=cache.get_k()
                v=cache.get_v()
            if cache.do_pag2 and controller.cur_self_layer in cache.self_layers and is_self and cache.injection:
                q_s=cache.get_q_s()
                    # k_s=k
                    # v_s=v
            ...
            if cache.do_pag and cache.do_cfg and not cache.do_pag2:
                hidden_states=torch.cat([hidden_states,hidden_states_con,hidden_states_uncon],dim=0)
            if cache.do_pag and cache.do_cfg and cache.do_pag2:
